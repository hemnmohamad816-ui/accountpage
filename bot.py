...
# ئایدی یان ناوی گروپی تیلیگرامەکەت (دەبێت بۆتەکە ئادمن بێت لەم گروپە)
CHANNEL_USERNAME = "@hararash1"
CHANNEL_LINK = "https://t.me/hararash1"

bot = telebot.TeleBot(BOT_TOKEN)

# فەرهەنگی زمانەکان (Kurdish, English, Arabic)
MESSAGES = {
    'ku': {
...
# فەرهەنگی هەڵگرتنی زمانی بەکارهێنەران
user_languages = {}
    ...
    if call.data == "check_sub":
        if is_subscribed(user_id):
            ...
            bot.edit_message_text(
                MESSAGES[lang]['joined'], 
                call.message.chat.id, 
                call.message.message_id,
                reply_markup=make_sub_keyboard(lang)
            )
        else:
        ...
        if is_subscribed(user_id):
            bot.edit_message_text(
                MESSAGES[new_lang]['joined'], 
                call.message.chat.id, 
                call.message.message_id,
                reply_markup=make_sub_keyboard(new_lang)
            )
        else:
...
def process_link(message):
    ...
    # پشکنینی داواکاری جۆینبوون بەر لە پرۆسەی داولۆندکردن
